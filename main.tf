  ...
  policy = jsonencode({
    ...
    Statement = [
        ...
        },
        {
            ...
            "Resource": "arn:aws:cognito-idp:${var.region}:${data.aws_caller_identity.current.account_id}:userpool/${aws_cognito_user_pool.file_download_management_pool.id}"
        }
    ]
  })
}
...
resource "null_resource" "modify_html" {
